...
class VideoRestorationService {
  ...
  static const unavailableMessage = 'Video export is not available in this build. Photo recovery remains available.';
  ...
  static bool get isBackendAvailableOnCurrentPlatform {
    return isBackendSupportedOnCurrentPlatform && _findFfmpegExecutable() != null;
  }
  ...
  Future<String> restoreVideo(String inputPath, RestorationSettings settings, {RawVideoDescriptor? rawDescriptor, VideoEditSettings trim = const VideoEditSettings(), ExportOptions exportOptions = const ExportOptions(), LutProfile lutProfile = LutProfile.none}) async {
    if (!isBackendSupportedOnCurrentPlatform) throw UnsupportedError(unavailableMessage);
    final ffmpeg = _findFfmpegExecutable();
    if (ffmpeg == null) throw StateError(backendUnavailableReason);

    final inputFile = File(inputPath);
    if (!await inputFile.exists()) throw FileSystemException('Input video not found.', inputPath);
    rawDescriptor?.validateForProcessing();
    trim.validate();

    final outputPath = await OutputPaths.forVideo(inputPath, extension: 'mp4');
    await File(outputPath).parent.create(recursive: true);
    final filters = settings.ffmpegFilters(extraFilters: LutService.videoFiltersFor(lutProfile)).join(',');
    ...
    if (exportOptions.keepAudio && rawDescriptor == null) args.addAll(['-map', '0:a?']);
    if (exportOptions.stripMetadata) args.addAll(['-map_metadata', '-1']);
    args.addAll(['-vf', filters, '-c:v', 'libx264', '-preset', 'medium', '-crf', '18', '-pix_fmt', 'yuv420p']);
    ...
    try {
      ...
      if (exitCode != 0) {
        throw StateError('Video restoration failed with FFmpeg exit $exitCode. ${_safeLog(log, inputPath, outputPath)}');
      }
    } finally {
  ...
  static String _safeLog(String log, String inputPath, String outputPath) {
    final redacted = log.replaceAll(inputPath, p.basename(inputPath)).replaceAll(outputPath, p.basename(outputPath)).replaceAll(RegExp(r'[\r\n]+'), ' ').trim();
